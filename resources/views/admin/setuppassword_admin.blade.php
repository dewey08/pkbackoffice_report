...
?>
        ...
        <form  method="post" action="{{ route('updatestatusuer.changpassword') }}"  onSubmit="return checkpass()">
        @csrf 
      ...
      <input type="password" name = "CHECK_NEWPASSWORD"  id="CHECK_NEWPASSWORD" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" >
        ...
        <button  type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('dashboard')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
         </form> 
        </div>
