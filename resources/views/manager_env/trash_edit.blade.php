     ...
     <div class="block" style="width: 95%;">
        <div class="block block-rounded block-bordered">
            <div class="block-header block-header-default">
                ...
                <div align="left">
                    <h3 class="block-title" style="font-family: 'Kanit', sans-serif;"><B>แก้ไขข้อมูลขยะ</B></h3>
                </div>

            </div>
        <div class="block-content block-content-full">
            <form action="{{ route('menv.trash_update') }}" method="post">
                @csrf       
                <input value="{{ $trashs->TRASH_ID}}" type="hidden" name="TRASH_ID" id="TRASH_ID" class="form-control input-lg" >
                <div class="row push">
                    <div class="col-sm-2 text-left">
                    <label>TRASH_NO :</label>
                    </div> 
                    <div class="col-lg-2">              
                    <input value="{{ $trashs->TRASH_BILL_NO}}" name="TRASH_BILL_NO" id="TRASH_BILL_NO"  class="form-control input-lg fo13" readonly>
                    </div> 
                    <div class="col-sm-1">
                        <label>วันที่บันทึก :</label>
                        </div> 
                        <div class="col-lg-2 ">              
                        <input value="{{formate($trashs->TRASH_DATE)}}" name="TRASH_DATE" id="TRASH_DATE" class="form-control input-lg datepicker fo13" data-date-format="mm/dd/yyyy" readonly >
                        </div>
                        <div class="col-sm-1 ">
                            <label>เวลา :</label>
                            </div> 
                            <div class="col-lg-1 ">              
                            <input value="{{ $trashs->TRASH_TIME}}" name="TRASH_TIME" id="TRASH_TIME" class="js-masked-time form-control fo13" >
                            </div> 
                    <div class="col-sm-1">
                    <label>ปี :</label>
                    </div> 
                    <div class="col-lg-1 "> 
                        <span>
                            ...
                            @if($budget->LEAVE_YEAR_ID== $trashs->TRASH_YEAR)
                        ...
                        <div class="row push">                           
                            <div class="col-sm-2 text-left">
                                ...
                                <div class="col-lg-5 ">
                                <span>
                                <select name="TRASH_SUP" id="TRASH_SUP" class="form-control input-lg js-example-basic-single fo13" >
                                    <option value="">--เลือก--</option>
                                    @foreach ($trash_supplis as $trash_suppli)
                                        
                                        @if($trash_suppli->VENDOR_ID == $trashs->TRASH_SUP)
                                        <option value="{{ $trash_suppli->VENDOR_ID  }}" selected>{{ $trash_suppli->VENDOR_NAME}} </option>
                                        @else
                                        <option value="{{ $trash_suppli->VENDOR_ID  }}">{{ $trash_suppli->VENDOR_NAME}} </option>
                                        @endif
                                    @endforeach 
                                    </select>
                                </span> 
                        ...
                        <div class="col-sm-1 ">
                            ...
                            <div class="col-lg-3 ">
                            <span>
                            <select name="TRASH_USER" id="TRASH_USER" class="form-control input-lg js-example-basic-single fo13">
                                <option value="">--เลือก--</option>
                                @foreach ($infopers as $infoper)
                                @if($infoper->ID == $trashs->TRASH_USER)
                                ...
                                <table class="gwt-table table-striped table-vcenter" style="width: 100%;">
                                    <thead style="background-color: #BDFBC9;">
                                        <tr height="40">
                                            ...
                                            <td style="text-align: center;font-family: 'Kanit', sans-serif;font-size: 14px;font-family: 'Kanit', sans-serif;font-size: 13px;" width="15%">น้ำหนัก</td>   
                                    ...
                                    <tbody class="tbody">
                                        <?php $number = 0; ?>
                                    @foreach($trash_subs as $trash_sub)
                                    <?php $number++;  ?>
                                        <tr height="20"> 
                                            
                                            <td style="text-align: center;font-family: 'Kanit', sans-serif;font-size: 13px;"> {{ $number}} </td>                                           
                                           <td>
                                                <input type="hidden" value="{{ $trash_sub->TRASH_SUB_IDID  }}" name="TRASH_SUB_IDID[]" id="TRASH_SUB_IDID[]" class="form-control input-sm fo13">
                                               
                                                <input value="{{ $trash_sub->TRASH_SUB_NAME  }}" name="" id="" class="form-control input-sm fo13" readonly>
                                           </td>
                                            <td>
                                                <input value="{{ $trash_sub->TRASH_SUB_QTY}}" name="TRASH_SUB_QTY[]" id="TRASH_SUB_QTY[]" class="form-control input-sm fo13" >
                                            </td>   
                                            <td>
                                                <input value="{{ $trash_sub->TRASH_SUB_UNIT}}" name="TRASH_SUB_UNIT[]" id="TRASH_SUB_UNIT[]" class="form-control input-sm fo13" readonly>
                                            </td>                               
