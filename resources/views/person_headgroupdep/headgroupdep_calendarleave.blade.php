            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="dropdown dropright">
                        <button type="button" style="background:#A2D2FF;font-family: 'Kanit', sans-serif;" class=" btn  dropdown-toggle" id="dropdown-dropright-primary" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            หน่วยงาน
                        </button>
                        <div class="dropdown-menu" aria-labelledby="dropdown-dropright-primary" x-placement="right-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(78px, 0px, 0px);">
                        <a class="dropdown-item devleave fs-18 " href="{{route('hgroupdep.headgroupdep_calendarleave')}}">ทั้งหมด</a>
                        @foreach($depinfos as $row)
                        <a class="dropdown-item pl-3 devleave"href="{{route('hgroupdep.headgroupdep_calendarleave').'?depid='.$row->HR_DEPARTMENT_ID.'&depname='.$row->HR_DEPARTMENT_NAME}}">{{$row->HR_DEPARTMENT_NAME}}</a>
                        @endforeach                    
                        </div>
                    </div>
                </div>
            </div><br>
             <a class="nav-main-link{{ request()->is('person_headgroupdep/headgroupdep_calendarleave') ? ' active' : '' }}" href="{{ url('person_headgroupdep/headgroupdep_leave') }}">
