...
class AuthenController extends Controller
{
    public function authen_dashboard(Request $request)
    {
        ...
        // $data_gethos = DB::connection('mysql3')->select('
        ...
        //         LIMIT 10
        // ');
        // foreach ($data_gethos as $key => $value) {
        //     $cid = $value->cid;
        //     $vn = $value->vn;
        //     $hn = $value->hn;
    ...
    public function authen_detail(Request $request,$dep)
    {
        $datadetail = DB::connection('mysql3')->select(' 
                SELECT o.main_dep,sk.department,o.vn as VN,vpa.visit_pttype_authen_cid,vpa.visit_pttype_authen_hn  
                ,vpa.visit_pttype_authen_auth_code,vpa.visit_pttype_authen_fullname,vpa.visit_pttype_authen_staff,vpa.mobile
                FROM ovst o 
                LEFT JOIN visit_pttype v on v.vn=o.vn
                LEFT JOIN vn_stat vs on vs.vn=o.vn
                LEFT JOIN visit_pttype_authen vpa on vpa.visit_pttype_authen_vn=o.vn
        ...
        <table style="width: 100%;" class="table table-hover table-striped table-bordered">
                    ...
                    foreach ($datadetail as $key => $value) {
                        ...
                        <td class="text-font" align="center" >'.$value->visit_pttype_authen_cid.'</td>
                        <td class="text-font" align="center" >'.$value->visit_pttype_authen_hn.'</td>
                        <td class="text-font text-pedding">'.$value->visit_pttype_authen_fullname.'</td>
                        ';
                        if ($value->visit_pttype_authen_auth_code > 0 ) {
                            $output.='<td class="text-font text-pedding"><label for="" style="color:black">'.$value->visit_pttype_authen_auth_code.'</label></td>';
                        } else {
                            $output.='<td class="text-font text-pedding" style="color:white;background-color: rgb(255, 58, 58)"><label for="" >'.$value->visit_pttype_authen_auth_code.'</label></td>';
                        }     
                                                
                        if ($value->mobile > 0 ) {
                            $output.='<td class="text-font" align="center" ><label for="" style="color:black">'.$value->mobile.'</label></td>';
                        } else {
                            $output.='<td class="text-font" align="center" style="color:white;background-color: rgb(255, 58, 58)" ><label for="" >'.$value->mobile.'</label></td>';
                        }  

                        $output.='                        
                        </td>
                        <td class="text-font text-pedding">'.$value->visit_pttype_authen_staff.'</td>
                        </tr>';
                        $count++;
                    }
